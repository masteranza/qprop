...
def preparePolar(filename):
    ...
    if data_type == 'complex':
        W = (x[5]+1.j*x[6]).reshape(Nk, Ntheta,
                ...
                for m in range(-l, l+1):
                    ...
                    for itheta in range(Ntheta):
                        for iphi in range(Nphi):
                            YLM = Ylm(
                                l, m, Theta[0, itheta, 0], Phi[0, 0, iphi])+0.j
...
def plot2D(filename):
    ...
    if polar_canvas == True:
        ...
        if Custom_ticks == True:
            ...
            plt.plot([0, 0], [R.min(), R.max()],
                     linewidth=1, color=TICKS_COLOR)
            plt.plot(np.zeros_like(TICKS), TICKS, '.', color=TICKS_COLOR)
            ax.set_rgrids(TICKS, angle=0, color=TICKS_COLOR, horizontalalignment='center',
    ...
    if polar_canvas == False:
        ...
        if cartesian == True:
            if polarization == 'xz':
                if wrt_energy == True:
                    plt.xlabel(r'${\bf k}_{\|} k_{\|}/2$' +
                               units_label, fontsize=FONTSIZE)
                    plt.ylabel(r'${\bf k}_{\perp} k_{\perp}/2$' +
                               units_label, fontsize=FONTSIZE)
                if wrt_energy == False:
            ...
            if polarization == 'xy':
                if wrt_energy == True:
                    plt.xlabel(r'${\bf k}_x k_x/2$' +
                               units_label, fontsize=FONTSIZE)
                    plt.ylabel(r'${\bf k}_y k_y/2$' +
                               units_label, fontsize=FONTSIZE)
                if wrt_energy == False:
        ...
        if cartesian == False:
            if polarization == 'xz':
                ...
                if Custom_ticks == True:
                    plt.xticks([0.5*pi*i for i in range(-2, 3, 1)],
                               fontsize=FONTSIZE)
                    ax.set_xticklabels(
                        [str(0.5*i)+r"$\pi$" for i in range(-2, 3, 1)], fontsize=FONTSIZE)
            if polarization == 'xy':
                plt.xlabel(r'$\varphi$', fontsize=FONTSIZE)
                if Custom_ticks == True:
                    plt.xticks([0.5*pi*i for i in range(0, 5, 1)],
                               fontsize=FONTSIZE)
                    ax.set_xticklabels(
