...
//Inits the wf to hydrogen like nlm initial state with charge Z.
void wavefunction::init(grid g, int n, int l, double Z)
{
	...
	if (g.dimens() != 34)
	{
		fprintf(stderr, "Init error: g.dimens() should be set to 34 or 44.\n");
		exit(-50);
	};
	...
	for (xindex = 0; xindex < g.ngps_x(); xindex++)
	{
		x = g.x(xindex);
		r = g.r(xindex);
		// if (r < 10) //COMMENT: this makes no sense
...
#else
				start[index_i] = r * sqrt(pow(Z * 2.0 / n, 3) * factorial(n - l - 1) / (2 * n * factorial(n + l))) * exp(-Z * r / n) * pow(2.0 * Z * r / n, l) * assoc_laguerre(n - l - 1, 2 * l + 1, 2.0 * Z * r / n);
#endif
			}
			else
			{
...
void wavefunction::add_analytic(grid g, int inittype, double w, fluid &ells, double mixture)
{
	long xindex, yindex, zindex, index_i, index, i;
	double x, y, z, r, ra;
	double offs = 0.0;
			...
			for (zindex = 0; zindex < g.ngps_z(); zindex++)
			{
				...
				case 1:
					if (ells[0] == yindex)
					{
						...
						{
						case 0:
							start[index_i] += mixture * r * 2.0 * exp(-ra) * sqrt(w) * w;
							if (r < 1)
							{
								fprintf(stdout, "ex0: n=%d l=%ld m=%ld\n", inittype, yindex, zindex);
							}
							break;
						default:
							// start[index_i] = cplxd(0.0, 0.0);
				...
				case 2:
					if (ells[0] == yindex)
					{
						...
						case 0:
							if (r < 1)
							{
								fprintf(stdout, "ex1: n=%d l=%ld m=%ld\n", inittype, yindex, zindex);
							}
							start[index_i] += mixture * r * 0.707106781 * (1.0 - 0.5 * ra) * exp(-0.5 * ra) * sqrt(w) * w;
							break;
						case 1:
							if (r < 1)
							{
								fprintf(stdout, "ex2: n=%d l=%ld m=%ld\n", inittype, yindex, zindex);
							}
							start[index_i] += mixture * r * 0.204124145 * ra * exp(-0.5 * ra) * sqrt(w) * w;
							break;
						default:
							// start[index_i] = cplxd(0.0, 0.0);
							break;
				...
				case 3:
					if (ells[0] == yindex)
					{
						...
						case 0:
							if (r < 1)
							{
								fprintf(stdout, "ex3: n=%d l=%ld m=%ld\n", inittype, yindex, zindex);
							}
							start[index_i] += mixture * r * 0.384900179 * (1 - 0.666666667 * ra + 0.074074074 * ra * ra) * exp(-0.333333333 * ra) * sqrt(w) * w;
							break;
						case 1:
							if (r < 1)
							{
								fprintf(stdout, "ex4: n=%d l=%ld m=%ld\n", inittype, yindex, zindex);
							}
							start[index_i] += mixture * r * 0.120962456 * ra * (1 - .166666667 * ra) * exp(-0.333333333 * ra) * sqrt(w) * w;
							break;
						case 2:
							if (r < 1)
							{
								fprintf(stdout, "ex5: n=%d l=%ld m=%ld\n", inittype, yindex, zindex);
							}
							start[index_i] += mixture * r * 0.009016009 * ra * ra * exp(-0.333333333 * ra) * sqrt(w) * w;
							break;
						default:
							// start[index_i] = cplxd(0.0, 0.0);
							break;
				...
				case 4:
					if (ells[0] == yindex)
					{
						...
						case 0:
							...
							start[index_i] += mixture * r * 0.25 * (1 - 0.75 * ra + 0.125 * ra * ra - 0.005208333 * ra * ra * ra) * exp(-0.25 * ra) * sqrt(w) * w;
							break;
						case 1:
							if (r < 1)
							{
							...
							start[index_i] += mixture * r * 0.080687153 * ra * (1 - 0.25 * ra + 0.0125 * ra * ra) * exp(-0.25 * ra) * sqrt(w) * w;
							break;
						case 2:
							if (r < 1)
							{
							...
							start[index_i] += mixture * r * .006987712 * ra * ra * (1 - 0.083333333 * ra) * exp(-0.25 * ra) * sqrt(w) * w;
							break;
						case 3:
							if (r < 1)
							{
							...
							start[index_i] += mixture * r * 0.000220092 * ra * ra * ra * exp(-0.25 * ra) * sqrt(w) * w;
							break;
						default:
							// start[index_i] = cplxd(0.0, 0.0);
							break;
...
void wavefunction::init_parabolic(grid g, int inittype, double w, int mu1, int mu2)
{
	long xindex, yindex, zindex, index_i, index, i;
	double x, y, z, r, ra;
	double offs = 0.0;
		...
		for (yindex = 0; yindex < g.ngps_y(); yindex++)
		{
			...
			for (zindex = 0; zindex < g.ngps_z(); zindex++)
			{
				...
				switch (inittype)
				{
					...
					// 		{
					// 		case 0:
					// 			start[index_i] = r * 2.0 * exp(-ra) * sqrt(w) * w;
					// 			if (r < 1)
					// 			{
					// 				fprintf(stdout, "ex0: n=%d l=%ld m=%ld\n", inittype, yindex, zindex);
					// 			}
					...
					// 				fprintf(stdout, "ex1: n=%d l=%ld m=%ld\n", inittype, yindex, zindex);
					// 			}
					// 			start[index_i] = r * 0.707106781 * (1.0 - 0.5 * ra) * exp(-0.5 * ra) * sqrt(w) * w;
					// 			break;
					// 		case 1:
					// 			if (r < 1)
					// 			{
					// 				fprintf(stdout, "ex2: n=%d l=%ld m=%ld\n", inittype, yindex, zindex);
					// 			}
					// 			start[index_i] = r * 0.204124145 * ra * exp(-0.5 * ra) * sqrt(w) * w;
					// 			break;
					// 		default:
					// 			start[index_i] = cplxd(0.0, 0.0);
					// 			break;
				...
				case 3:
					if (yindex <= 2)
					{
						...
						case 0:
							...
							start[index_i] += mult * r * 0.384900179 * (1 - 0.666666667 * ra + 0.074074074 * ra * ra) * exp(-0.333333333 * ra) * sqrt(w) * w;
							break;
						case 1:
							if (mu1 == 1 && mu2 == -1)
							{
							...
							start[index_i] += mult * r * 0.120962456 * ra * (1 - .166666667 * ra) * exp(-0.333333333 * ra) * sqrt(w) * w;
							break;
						case 2:
							if (mu1 == 1 && mu2 == -1)
							{
							...
							start[index_i] += mult * r * 0.009016009 * ra * ra * exp(-0.333333333 * ra) * sqrt(w) * w;
							break;
						default:
							start[index_i] = cplxd(0.0, 0.0);
							break;
				...
				// 				fprintf(stdout, "ex3: n=%d l=%ld m=%ld\n", inittype, yindex, zindex);
				// 			}
				// 			start[index_i] = r * 0.25 * (1 - 0.75 * ra + 0.125 * ra * ra - 0.005208333 * ra * ra * ra) * exp(-0.25 * ra) * sqrt(w) * w;
				// 			break;
				// 		case 1:
				// 			if (r < 1)
				// 			{
				// 				fprintf(stdout, "ex4: n=%d l=%ld m=%ld\n", inittype, yindex, zindex);
				// 			}
				// 			start[index_i] = r * 0.080687153 * ra * (1 - 0.25 * ra + 0.0125 * ra * ra) * exp(-0.25 * ra) * sqrt(w) * w;
				// 			break;
				// 		case 2:
				// 			if (r < 1)
				// 			{
				// 				fprintf(stdout, "ex5: n=%d l=%ld m=%ld\n", inittype, yindex, zindex);
				// 			}
				// 			start[index_i] = r * .006987712 * ra * ra * (1 - 0.083333333 * ra) * exp(-0.25 * ra) * sqrt(w) * w;
				// 			break;
				// 		case 3:
				// 			if (r < 1)
				// 			{
				// 				fprintf(stdout, "ex5: n=%d l=%ld m=%ld\n", inittype, yindex, zindex);
				// 			}
				// 			start[index_i] = r * 0.000220092 * ra * ra * ra * exp(-0.25 * ra) * sqrt(w) * w;
				// 			break;
				// 		default:
				// 			start[index_i] = cplxd(0.0, 0.0);
				// 			break;
...
void wavefunction::init_rlm(grid g, int n, int l, int m, double Z)
{
	long xindex, yindex, zindex, index_i, index, i, mi;
	double r;
	double offs = 0.0;
	double realpart, imagpart;
	long ctrl_id;
	...
	for (xindex = 0; xindex < g.ngps_x(); xindex++)
	{
		...
		for (yindex = 0; yindex < g.ngps_y(); yindex++)
		{
			for (mi = -yindex; mi <= yindex; mi++)
			{
				index = g.rlmindex(xindex, yindex, mi);
				...
				if ((yindex == l) && (mi == m))
				{
					...
					start[index_i] = cplxd(-rand() / (RAND_MAX + 1.0) + 0.5, 0.0);
#else
					start[index_i] = sqrt(pow(Z * 2 / n, 3) * factorial(n - l - 1) / (2 * n * factorial(n + l))) * exp(-Z * r / n) * pow(2 * Z * r / n, l) * assoc_laguerre(n - l - 1, 2 * l + 1, 2 * Z * r / n);
#endif
				}
				else
				{
...
int wavefunction::init(grid g, FILE *file, int output_of_interest, int iv)
{
	...
	{
		if (iv == 1)
			fprintf(stdout, "Ignoring of %ld...", i);
...
void wavefunction::regrid_adding(grid g, grid g_small, const wavefunction &v, double weight)
{
	long xindex, yindex, zindex, index, index_small,
		xupper, yupper, zupper, m_limit, mindex;

	xupper = g_small.ngps_x() > g.ngps_x() ? g.ngps_x() : g_small.ngps_x();
	zupper = g_small.ngps_z() > g.ngps_z() ? g.ngps_z() : g_small.ngps_z();
	yupper = g_small.ngps_y() > g.ngps_y() ? g.ngps_y() : g_small.ngps_y();

	for (xindex = 0; xindex < xupper; xindex++)
	{
		for (yindex = 0; yindex < yupper; yindex++)
		{
			m_limit = g.dimens() == 34 ? 0 : yindex;
			...
			for (mindex = -m_limit; mindex <= m_limit; mindex++)
			{
				for (zindex = 0; zindex < zupper; zindex++)
				{
					...
					start[index] += weight * v[index_small];
				}
			}
		}
	}
...
{
	for (long r = 0; r < v.wf_size(); r++)
		start[g.index(r, l, m, i)] = v[r];
}
...
void wavefunction::extract_xy2(FILE *os, grid g, long zindex)
{
								  ...
								  double charge)
{
	...
	for (ell = 0; ell < g.ngps_y() - 1; ell++)
	{
		for (m = -ell; m <= ell; m++)
		{
			...
			blm = sqrt((ell - m + 1.0) / ((2.0 * ell + 1) * (2.0 * ell + 3))) * (m * sqrt(ell - m + 2) - sqrt((ell + m + 1) * ((ell + 1) * (ell + 2) - m * (m - 1))));
			...
			xindex = 0;
			tmptmp_one[xindex] = (cornerpart + x * zeta * dlm) * tmp_one[xindex] + (OOS + zeta * dlm) * tmp_one[xindex + 1];
			for (xindex = 1; xindex < g.ngps_x() - 1; xindex++)
			{
				tmptmp_one[xindex] = (OOS - zeta * dlm) * tmp_one[xindex - 1] + TOT * tmp_one[xindex] + (OOS + zeta * dlm) * tmp_one[xindex + 1];
			};
			xindex = g.ngps_x() - 1;
			tmptmp_one[xindex] = (OOS - zeta * dlm) * tmp_one[xindex - 1] + (cornerpart - x * zeta * dlm) * tmp_one[xindex];
			// -------- end of calculating Yoverlminusone times Gamma_1

			// ------- calculate Yoverlminustwo times Gamma_2
			xindex = 0;
			tmptmp_two[xindex] = (cornerpart - x * zeta * dlm) * tmp_two[xindex] + (OOS - zeta * dlm) * tmp_two[xindex + 1];
			for (xindex = 1; xindex < g.ngps_x() - 1; xindex++)
			{
				tmptmp_two[xindex] = (OOS + zeta * dlm) * tmp_two[xindex - 1] + TOT * tmp_two[xindex] + (OOS - zeta * dlm) * tmp_two[xindex + 1];
			};
			xindex = g.ngps_x() - 1;
			tmptmp_two[xindex] = (OOS + zeta * dlm) * tmp_two[xindex - 1] + (cornerpart + x * zeta * dlm) * tmp_two[xindex];
			...
			btildelm = sqrt((ell + m + 1.0) / ((2.0 * ell + 1) * (2.0 * ell + 3))) * (m * sqrt(ell + m + 2) + sqrt((ell - m + 1) * ((ell + 1) * (ell + 2) - m * (m + 1))));
			...
			xindex = 0;
			tmptmp_one[xindex] = (cornerpart - x * zeta * dtildelm) * tmp_one[xindex] + (OOS - zeta * dtildelm) * tmp_one[xindex + 1];
			for (xindex = 1; xindex < g.ngps_x() - 1; xindex++)
			{
				tmptmp_one[xindex] = (OOS + zeta * dtildelm) * tmp_one[xindex - 1] + TOT * tmp_one[xindex] + (OOS - zeta * dtildelm) * tmp_one[xindex + 1];
			};
			xindex = g.ngps_x() - 1;
			tmptmp_one[xindex] = (OOS + zeta * dtildelm) * tmp_one[xindex - 1] + (cornerpart + x * zeta * dtildelm) * tmp_one[xindex];
			// -------- end of calculating Yoverlminustildeone times Gamma_1tilde

			// ------- calculate Yoverlminustildetwo times Gamma_2tilde
			xindex = 0;
			tmptmp_two[xindex] = (cornerpart + x * zeta * dtildelm) * tmp_two[xindex] + (OOS + zeta * dtildelm) * tmp_two[xindex + 1];
			for (xindex = 1; xindex < g.ngps_x() - 1; xindex++)
			{
				tmptmp_two[xindex] = (OOS - zeta * dtildelm) * tmp_two[xindex - 1] + TOT * tmp_two[xindex] + (OOS + zeta * dtildelm) * tmp_two[xindex + 1];
			};
			xindex = g.ngps_x() - 1;
			tmptmp_two[xindex] = (OOS - zeta * dtildelm) * tmp_two[xindex - 1] + (cornerpart - x * zeta * dtildelm) * tmp_two[xindex];
	...
	for (yindex = 0; yindex < g.ngps_y(); yindex++)
	{
		for (m = -yindex; m <= yindex; m++)
		{
			...
			if (yindex == 0)
			{
				rhsone[index] = (aaaa + halfimagitimestepOOS * (staticpot[index_xp])) * start[index_xp] + ((cplxd)M_two_upperleft * (1.0 - halfimagitimestep * (staticpot[index])) - imagi * (cplxd)Delta_two_upperleft * (cplxd)0.5 * timestep) * start[index];
			}
			else
			{
				rhsone[index] = (aaaa + halfimagitimestepOOS * (staticpot[index_xp])) * start[index_xp] + (bbbb + halfimagitimestepFOT * (staticpot[index])) * start[index];
			};
			...
			for (xindex = 1; xindex < g.ngps_x() - 1; xindex++)
			{
				...
				rhsone[index] = (aaaa + halfimagitimestepOOS * (staticpot[index_xp])) * start[index_xp] + (bbbb + halfimagitimestepFOT * (staticpot[index])) * start[index] + (cccc + halfimagitimestepOOS * (staticpot[index_xm])) * start[index_xm];
			};

			xindex = g.ngps_x() - 1;
			index = g.rlmindex(xindex, yindex, m);
			index_xm = g.rlmindex(xindex - 1, yindex, m);
			rhsone[index] = (bbbb + halfimagitimestepFOT * (staticpot[index])) * start[index] + (cccc + halfimagitimestepOOS * (staticpot[index_xm])) * start[index_xm];
		};
	};
	...
	for (yindex = 0; yindex < g.ngps_y(); yindex++)
	{
		for (m = -yindex; m <= yindex; m++)
		{
			...
			index = g.rlmindex(xindex, yindex, m);
			index_xm = g.rlmindex(xindex - 1, yindex, m);
			aa[xindex] = aaaa - halfimagitimestepOOS * (hamil.scalarpotx(r + g.delt_x(), yindex, 0.0, time, me) - imagi * (cplxd)hamil.imagpot(xindex, yindex, 0, time, g));
			cc[xindex] = (cccc - halfimagitimestepOOS * (staticpot[index_xm]));
			bb[xindex] = (bbbb - halfimagitimestepFOT * (staticpot[index]));
			tmp_one[xindex] = rhsone[index];
...
*/
void wavefunction::dump_to_file_sh(grid g, FILE *os, int stepwidth, int iv =false)
{
	long index;
...
wavefunction wavefunction::calculate_Theta(grid g, const fluid &degeneracies, const fluid &ms)
{
	...
	if (g.ngps_y() < 2)
		fprintf(stdout, "warn: calculate_Theta: g.ngps_y()<2\n");
		...
		for (zindex = 0; zindex < g.ngps_z(); zindex++)
		{
			...
			if (g.ngps_y() > (long)(fabs(m)))
			{
				...
				for (lindex = (long)(fabs(m)) + 1; lindex < g.ngps_y() - 1; lindex++)
				{
					...
					tmp += (cplxd)(sqrt(((lindex + 1.0) * (lindex + 1.0) - m * m) / ((2.0 * lindex + 1.0) * (2.0 * lindex + 3.0)))) * conj(start[indexlpo]) * start[index] + (cplxd)(sqrt((lindex * lindex - m * m) / ((2.0 * lindex + 1.0) * (2.0 * lindex - 1.0)))) * conj(start[indexlmo]) * start[index];
				};

				lindex = g.ngps_y() - 1;
				index = g.index(rindex, lindex, zindex);
				indexlmo = g.index(rindex, lindex - 1, zindex);
				tmp += (cplxd)(sqrt((lindex * lindex - m * m) / ((2.0 * lindex + 1.0) * (2.0 * lindex - 1.0)))) * conj(start[indexlmo]) * start[index];
			};
	...
	case 34:
		for (zindex = 0; zindex < g.ngps_z(); zindex++)
		{
			for (xindex = 0; xindex < g.ngps_x(); xindex++)
			{
				...
				{
					index = g.index(xindex, yindex, zindex);
					start[index] = hamil.scalarpot(x, yindex, 0.0, 0.0, me) + 0.5 * yindex * (yindex + 1) / (x * x) - imagi * (cplxd)hamil.imagpot(xindex, yindex, 0, time, g);
				};
			};
		};
		break;
			...
			for (ell = 0; ell < g.ngps_y(); ell++)
			{
				for (m = -ell; m <= ell; m++)
				{
					...
					start[index] = hamil.scalarpot(x, ell, 0.0, 0.0, me) +
								   0.5 * ell * (ell + 1) / (x * x) - imagi * (cplxd)hamil.imagpot(xindex, ell, 0, time, g);
				};
			};
		};
		break;
	...
	case 34:
		...
		for (zindex = 0; zindex < g.ngps_z(); zindex++)
		{
			...
			for (yindex = 0; yindex < g.ngps_y(); yindex++)
			{
				...
				if (yindex == 0)
				{
					rhsone[index] = (aaaa - (staticpot[index_xp]) / 6.0) * start[index_xp] + ((cplxd)M_two_upperleft * (staticpot[index]) + Delta_two_upperleft) * start[index];
				}
				else
				{
					rhsone[index] = (aaaa - (staticpot[index_xp]) / 6.0) * start[index_xp] + (bbbb - (cplxd)FOT * (staticpot[index])) * start[index];
				};
				...
				for (xindex = 1; xindex < g.ngps_x() - 1; xindex++)
				{
					...
					index_xp = g.index(xindex + 1, yindex, zindex);
					index_xm = g.index(xindex - 1, yindex, zindex);
					rhsone[index] = (aaaa - (staticpot[index_xp]) / 6.0) * start[index_xp] + (bbbb - (cplxd)FOT * (staticpot[index])) * start[index] + (cccc - (staticpot[index_xm]) / 6.0) * start[index_xm];
				};

				xindex = g.ngps_x() - 1;
				index = g.index(xindex, yindex, zindex);
				index_xm = g.index(xindex - 1, yindex, zindex);
				rhsone[index] = (bbbb - (cplxd)FOT * (staticpot[index])) * start[index] + (cccc - (staticpot[index_xm]) / 6.0) * start[index_xm];
			};
	...
	case 34:
		for (orb_no = 0; orb_no < g.ngps_z(); orb_no++)
		{
			...
			for (yindex = 0; yindex < g.ngps_y(); yindex++)
			{
				...
				if (yindex == 0)
				{
					rhsone[index] = (aaaa - (staticpot[index_xp]) / 6.0) * start[index_xp] + (M_two_upperleft * (staticpot[index]) + Delta_two_upperleft) * start[index];
				}
				else
				{
					rhsone[index] = (aaaa - (staticpot[index_xp]) / 6.0) * start[index_xp] + (bbbb - FOT * (staticpot[index])) * start[index];
				};
				...
				for (xindex = 1; xindex < g.ngps_x() - 1; xindex++)
				{
					...
					index_xp = g.index(xindex + 1, yindex, zindex);
					index_xm = g.index(xindex - 1, yindex, zindex);
					rhsone[index] = (aaaa - (staticpot[index_xp]) / 6.0) * start[index_xp] + (bbbb - FOT * (staticpot[index])) * start[index] + (cccc - (staticpot[index_xm]) / 6.0) * start[index_xm];
				};

				xindex = g.ngps_x() - 1;
				index = g.index(xindex, yindex, zindex);
				index_xm = g.index(xindex - 1, yindex, zindex);
				rhsone[index] = (bbbb - FOT * (staticpot[index])) * start[index] + (cccc - (staticpot[index_xm]) / 6.0) * start[index_xm];
			};
	...
	case 34:
		for (orb_no = 0; orb_no < g.ngps_z(); orb_no++)
		{
			...
			for (xindex = 0; xindex < g.ngps_x(); xindex++)
			{
				...
				{
					index = g.index(xindex, yindex, zindex);
					result[zindex] = result[zindex] + conj(start[index]) * wf_one[xindex] * start[index] * g.delt_x();
				};
			};
		};
		break;
...
void wavefunction::extract_ell_m(grid g, long ell, long m0)
{
	...
	{
	case 34:
		for (yindex = m0; yindex < g.ngps_y(); yindex++)
		{
			if (yindex != ell)
			{
				for (xindex = 0; xindex < g.ngps_x(); xindex++)
		...
		for (zindex = 0; zindex < g.ngps_z(); zindex++)
		{
			for (xindex = 0; xindex < g.ngps_x(); xindex++)
			{
				...
				for (yindex = 1; yindex < g.ngps_y() - 1; yindex++)
				{
					...
					result = result + (cplxd)g.delt_x() * (cplxd)r * start[index] * (conj(start[index_yp]) * (cplxd)(yindex + 1.0) / sqrt((2.0 * yindex + 1.0) * (2.0 * yindex + 3.0)) + conj(start[index_ym]) * (cplxd)yindex / sqrt((2.0 * yindex - 1.0) * (2.0 * yindex + 1.0)));
				};
...
cplxd wavefunction::expect_z(grid g, wavefunction &orig) // attention!!! works for m=0 states only because m-dependence is suppressed
{
	cplxd result(0.0, 0.0);
	long xindex, yindex, zindex;
	long index, index_ym, index_yp;
	...
	case 34:
		for (zindex = 0; zindex < g.ngps_z(); zindex++)
		{
			for (xindex = 0; xindex < g.ngps_x(); xindex++)
			{
				...
				result = result + conj(orig[index_yp]) * (cplxd)r * start[index] * (cplxd)(yindex + 1.0) / sqrt((2.0 * yindex + 1.0) * (2.0 * yindex + 3.0)) * (cplxd)g.delt_x();
				...
				for (yindex = 1; yindex < g.ngps_y() - 1; yindex++)
				{
					...
					result = result + (cplxd)g.delt_x() * (cplxd)r * start[index] * (conj(orig[index_yp]) * (cplxd)(yindex + 1.0) / sqrt((2.0 * yindex + 1.0) * (2.0 * yindex + 3.0)) + conj(orig[index_ym]) * (cplxd)yindex / sqrt((2.0 * yindex - 1.0) * (2.0 * yindex + 1.0)));
				};
				...
				result = result + (cplxd)(g.delt_x() * r) * start[index] * conj(orig[index_ym]) * (cplxd)yindex / sqrt((2.0 * yindex - 1.0) * (2.0 * yindex + 1.0));
			};
		};
...
// ***** MJ
// single orbital case (lin. pol. only); overloaded below
// computes dipole moment part from L and L+1
cplxd wavefunction::expect_z(grid g, int L) // attention!!! works for m=0 states only because m-dependence is suppressed
{
	cplxd result(0.0, 0.0);
	long xindex, yindex, zindex;
	long index, index_yp;
	...
	case 34:
		for (zindex = 0; zindex < g.ngps_z(); zindex++)
		{
			for (xindex = 0; xindex < g.ngps_x(); xindex++)
			{
				...
				result = result + (real(start[index_yp]) * real(start[index]) + imag(start[index_yp]) * imag(start[index])) * r * 2.0 * (yindex + 1.0) / sqrt((2.0 * yindex + 1.0) * (2.0 * yindex + 3.0)) * g.delt_x();
			};
		};
			...
			for (xindex = 0; xindex < g.ngps_x(); xindex++)
			{
				if (g.ngps_y() > 1)
				{
					...
					for (yindex = 1; yindex < g.ngps_y() - 1; yindex++)
					{
						...
						result = result + g.delt_x() * degeneracies[zindex] * r * start[index] * ((conj(start[index_yp])) * (sqrt((yindex + 1.0) * (yindex + 1) - ms[zindex] * ms[zindex])) / sqrt((2.0 * yindex + 1.0) * (2.0 * yindex + 3.0)) + (conj(start[index_ym])) * (sqrt(yindex * yindex - ms[zindex] * ms[zindex])) / sqrt((2.0 * yindex - 1.0) * (2.0 * yindex + 1.0)));
					};
	...
	default:
		for (zindex = 0; zindex < g.ngps_z(); zindex++)
		{
			...
			for (xindex = 0; xindex < g.ngps_x(); xindex++)
			{
				...
				{
					index = g.index(xindex, yindex, zindex);
					result = result + z * real(conj(start[index]) * start[index]) * g.delt_x() * g.delt_y() * g.delt_z();
				};
			};
		};
	};
...
cplxd wavefunction::accel_z(grid g, int m, hamop hamil, double time, int me, double deltat)
{
	...
	{
	case 34:
		// #pragma omp parallel for default(none) private(ell_p, xindex, ell_m, index, dU_dr, r, ell) shared(g, hamil, result, time, m, me, dr, Nx, Ez)
		for (zindex = 0; zindex < g.ngps_z(); zindex++)
		{
			// cplxd local_result(0.0, 0.0);
			for (xindex = 0; xindex < Nx; xindex++)
			{
				...
				for (ell = 1; ell < g.ngps_y() - 1; ell++)
				{
					...
					result += (cplxd)(-dU_dr) * start[index] *
							  (conj(start[ell_p]) * (cplxd)sqrt((ell + 1.0) * (ell + 1.0) - m * m) / sqrt((2.0 * ell + 1.0) * (2.0 * ell + 3.0)) + conj(start[ell_m]) * (cplxd)sqrt(ell * ell - m * m) / sqrt((2.0 * ell - 1.0) * (2.0 * ell + 1.0)));
				};
...
cplxd wavefunction::expect_cycl_pol_minus(grid g)
{
	...
	case 44:
		for (xindex = 0; xindex < g.ngps_x(); xindex++)
		{
			...
			result = result + r * conj(start[index]) * (-start[index_lpmp] * sqrt((ell + 1.0) * (ell + m + 1.0) * (ell + m + 2.0) / ((2.0 * ell + 1) * (2.0 * ell + 2) * (2.0 * ell + 3))));
			...
			for (ell = 1; ell < g.ngps_y() - 1; ell++)
			{
				for (m = -ell; m <= ell; m++)
				{
					...
					if (abs(m + 1) <= ell - 1)
					{
						...
						result = result + r * conj(start[index]) * (start[index_lmmp] * sqrt((ell - m - 1.0) * (ell - m) / (2.0 * (2.0 * ell - 1) * (2.0 * ell + 1))));
					};
					if (abs(m + 1) <= ell + 1)
					{
						index_lpmp = g.rlmindex(xindex, ell + 1, m + 1);
...
cplxd wavefunction::expect_rr(grid g, fluid &degeneracies)
{
	...
	for (zindex = 0; zindex < g.ngps_z(); zindex++)
	{
		for (xindex = 0; xindex < g.ngps_x(); xindex++)
		{
			...
			{
				index = g.index(xindex, yindex, zindex);
				result = result + g.delt_x() * degeneracies[zindex] * start[index] * conj(start[index]) * r * r;
			};
		};
	};
