...
class tsurffSaveWF
{
    ...
    ofstream dpsi_dr;
    long ind_R;
    // (let ((mylist '(("double" "delta-r" "param_ini") ("double" "R-tsurff" "param_tsurff") ("long" "qprop-dim" "param_ini") ("long" "l-grid-size" "param_prop") ))) (param-list-for-class mylist))
    /// \f$\RI\f$
...
public:
        ...
        : psi(dir_name + ts_fname + string("-psi.raw"), std::ios::binary), dpsi_dr(dir_name + ts_fname + string("-dpsidr.raw"), std::ios::binary)
    {
        ...
        // re_l_grid_size = param_prop.getLong("l-grid-size");
    ...
    */
    void operator()(const wavefunction &wf)
    {
        ...
        if (qprop_dim == 44)
        {
            for (long i_ell = 0; i_ell < re_l_grid_size; i_ell++)
            {
...
public:
    ...
    */
    ...
    void partial_amplitudes()
    {

        ofstream tsurff_partial_dat(dir_name + ts_fname + "-partial" + fixed_length_number(num_proc, i_proc) + ".dat");
        ofstream isurfv_partial_dat(dir_name + is_fname + "-partial" + fixed_length_number(num_proc, i_proc) + ".dat");
        double norm_a_ell_k;
...
private:
    ...
    void prep_mom_grid()
    {
        ...
        if (num_theta_surff > 1)
        {
            ...
            const double dtheta = M_PI / double(num_theta_surff - 1);
            for (long i_theta = 0; i_theta < num_theta_surff; i_theta++)
            {
                thetas_surff[i_theta] = (i_theta)*dtheta;
            };
        };
...
#ifdef HAVE_MPI
                psi_buffer_R[2 * count] = real(psi_R[count]);
                psi_buffer_R[2 * count + 1] = imag(psi_R[count]);
                psi_buffer_R_deriv[2 * count] = real(psi_R_deriv[count]);
                psi_buffer_R_deriv[2 * count + 1] = imag(psi_R_deriv[count]);
#endif
    ...
    */
    ...
    {
        return i_ell + i_theta * re_l_grid_size + i_k * re_l_grid_size * num_theta_surff;
    };
        ...
        for (long i_k = 0; i_k < num_k_proc; i_k++)
        {
            ...
            for (long i_theta = 0; i_theta < num_theta_surff; i_theta++)
            {
                ...
                for (long i_t = start_t; i_t < stop_t; i_t++)
                {
                    ...
                    const double time = dt * double(i_t);
                    ...
                    for (long i_ell = 0; i_ell < re_l_grid_size; i_ell++)
                    {
                        ...
                        const long ind_wave = i_ell + cache_i_t * re_l_grid_size;
    ...
    */
    ...
    {
        return i_ell1 + i_ell2 * re_l_grid_size + i_k * re_l_grid_size * re_l_grid_size;
    };
    ...
    */
    ...
    {
        // time integration for t-surff
        double j_l2kalpha[re_l_grid_size + 1];
        for (long i_k = 0; i_k < num_k_proc; i_k++)
        {
            const double k = k_values[i_k];
            for (long i_t = start_t; i_t < stop_t; i_t++)
            {
                ...
                for (long i_ell2 = 0; i_ell2 < re_l_grid_size; i_ell2++)
                {
                    for (long i_ell = 0; i_ell < re_l_grid_size; i_ell++)
                    {
    ...
    {
        const long re_l_grid_size_2 = re_l_grid_size * re_l_grid_size;
        return ((i_ell1 + 1) * (i_ell1 + 1) - (i_ell1 + 1) + i_m1) + ((i_ell2 + 1) * (i_ell2 + 1) - (i_ell2 + 1) + i_m2) * re_l_grid_size_2 + i_k * re_l_grid_size_2 * re_l_grid_size_2;
    };

    inline long index_ell1_m1_i_t(long i_ell1, long i_m1, long i_t) const
    {
        const long re_l_grid_size_2 = re_l_grid_size * re_l_grid_size;
        return ((i_ell1 + 1) * (i_ell1 + 1) - (i_ell1 + 1) + i_m1) + i_t * re_l_grid_size_2;
    };
            ...
            for (long i_t = start_t; i_t < stop_t; i_t++)
            {
                ...
                for (long i_ell2 = 0; i_ell2 < re_l_grid_size; i_ell2++)
                {
                    for (long i_m2 = -i_ell2; i_m2 < i_ell2 + 1; i_m2++)
                    {
                        ...
                        for (long i_ell1 = 0; i_ell1 < re_l_grid_size; i_ell1++)
                        {
    ...
    */
    ...
    inline void fill_bessel_array(double k, double R, double *j_lkr)
    {
        ...
        long max_ell_bessel(re_l_grid_size + 1);
        ...
        for (long i_ell = i_ell1; i_ell < re_l_grid_size + 1; i_ell++)
        {
            j_lkr[i_ell] = 0.0;
        };
        ...
        if (a_sign < 0.0)
        {
            for (long i_ell = 1; i_ell < re_l_grid_size + 1; i_ell += 2)
            {
                j_lkr[i_ell] *= -1.0;
            };
        };
    ...
    */
    ...
    {
        if (i_proc == 0)
            logAdd("Obtaining angle-resolved spectrum.\n");
...
#endif
        // In order to calculate the derivative of the spherical Bessel function j_l we use the j_{l+1} -> size of the array re_l_grid_size+1
        double j_lkr[re_l_grid_size + 1];
        // Big loop for summing over ells and such
        ofstream tsurff_polar_dat(dir_name + ts_fname + "-polar" + fixed_length_number(num_proc, i_proc) + ".dat");
        tsurff_polar_dat.precision(17);
        ofstream isurfv_polar_dat(dir_name + is_fname + "-polar" + fixed_length_number(num_proc, i_proc) + ".dat");
        isurfv_polar_dat.precision(17);
        for (long i_k = 0; i_k < num_k_proc; i_k++)
        {
            if ((i_proc == 0) && (i_k % 100 == 0))
                logAdd("%d k values of %d complete\n", i_k, num_k_proc);
            double k;
            double energy;
            cplxd psi_R_inf[re_l_grid_size], psi_deriv_R_inf[re_l_grid_size];
#ifndef EXTRA_TIME_TSURFF_ONLY
            {
                long fscanf_status;
                double psi_re, psi_im, d_psi_dr_re, d_psi_dr_im;
            ...
            for (long i_theta = 0; i_theta < num_theta_surff; i_theta++)
            {
                ...
                for (long ell = labs(re_initial_m); ell < re_l_grid_size; ell++)
                {
                    // $c_{\ell, m}=\sqrt{\frac{(\ell+1)^2-m^2}{(2\ell+1)(2\ell+3)}}$
                    double c_ell_m = sqrt(pow2(ell + 1) - pow2(re_initial_m)) * sqrt(1. / double((2 * ell + 1) * (2 * ell + 3)));
                    ...
                    const cplxd Ylm = Ylm_prefactor(re_initial_m) * gsl_sf_legendre_sphPlm(ell, labs(re_initial_m), cos_theta);
                    const cplxd Ylp1m = Ylm_prefactor(re_initial_m) * gsl_sf_legendre_sphPlm(ell + 1, labs(re_initial_m), cos_theta);
                    ...
                    const cplxd b_kl_psi_1 = ((ell + 1) < re_l_grid_size) ? (cplxd(0., 2.0) * c_ell_m * j_lkr[ell] * Ylm * psi_A_surff[ind_surff + 1]) : 0.0;
                    const cplxd b_kl = b_kl_Ylm_1 + b_kl_psi_1 + (j_lkr[ell] * (psi_deriv_surff[ind_surff] - psi_surff[ind_surff] / R_tsurff) - j_lkr_deriv * psi_surff[ind_surff]) * Ylm;

                    b_k += pow_neg_i(ell + 1) * b_kl;

#ifndef EXTRA_TIME_TSURFF_ONLY
                    {
                        const cplxd b_kl_analytic = (j_lkr[ell] * (psi_deriv_R_inf[ell] - psi_R_inf[ell] / R_tsurff) - j_lkr_deriv * psi_R_inf[ell]) * Ylm;
                ...
                b_k *= R_tsurff / sqrt(2. * M_PI) * dt;
                tsurff_polar_dat << energy << " " << k << " " << theta << " "
                                 << "0"
                                 << " " << norm(b_k) * k << " " << real(b_k) << " " << imag(b_k) << endl;
#ifndef EXTRA_TIME_TSURFF_ONLY
                {
                    b_k_analytic *= R_tsurff / sqrt(2. * M_PI) * cplxd(0.0, 1.0) * exp(cplxd(0.0, 1.0) * energy * duration);
                    b_k += b_k_analytic;
                    isurfv_polar_dat << energy << " " << k << " " << theta << " "
                                     << "0"
                                     << " " << norm(b_k) * k << " " << real(b_k) << " " << imag(b_k) << endl;
                };
#endif
    ...
    */
    void eval_spectrum_44_theta_phi()
    {
        ...
        for (long i_k = 0; i_k < num_k_proc; i_k++)
        {
            ...
            double k;
            double energy;
            cplxd psi_R_inf[re_l_grid_size * re_l_grid_size], psi_deriv_R_inf[re_l_grid_size * re_l_grid_size];
#ifndef EXTRA_TIME_TSURFF_ONLY
            {
                long fscanf_status;
                double psi_re, psi_im, d_psi_dr_re, d_psi_dr_im;
                ...
                for (long i_phi = 0; i_phi < num_phi_surff; i_phi++)
                {
                    ...
                    for (long i_ell = 0; i_ell < re_l_grid_size; i_ell++)
                    {
                        ...
                        for (long i_m = -long(i_ell); i_m < long(i_ell + 1); i_m++)
                        {
                            ...
                            const long N_ell = re_l_grid_size;
                            ...
                            b_k += pow_neg_i(i_ell + 1) * j_lkr[i_ell] * Ylm * (-1.0 / R_tsurff * psi_surff[ind_surff] + psi_deriv_surff[ind_surff]) - pow_neg_i(i_ell + 1) * j_lkr_deriv * Ylm * psi_surff[ind_surff];

#ifndef EXTRA_TIME_TSURFF_ONLY
                            {
                                long lmindex = i_ell * (i_ell + 1) + i_m;
                                b_k_analytic += pow_neg_i(i_ell + 1) * (j_lkr[i_ell] * (psi_deriv_R_inf[lmindex] - 1.0 / R_tsurff * psi_R_inf[lmindex]) - j_lkr_deriv * psi_R_inf[lmindex]) * Ylm;
                            };
#endif
                        };
                    };
    ...
    {
        return i_ell1 + i_ell2 * re_l_grid_size;
    };
    ...
    */
    ...
    {
        return i_ell + i_k * re_l_grid_size;
    };
    ...
    */
    void eval_partial_spectra_34_ell1_ell2()
    {
        if (i_proc == 0)
            ...
            logAdd("Obtaining partial-amplitude-resolved spectrum.\n");
        // In order to calculate the derivative of the spherical Bessel function j_l we use the j_{l+1} -> size of the array re_l_grid_size+1
        double j_lkr[re_l_grid_size + 1];
        double_ptr wigner_pre(new double[re_l_grid_size * re_l_grid_size]);
        for (long i_ell = labs(re_initial_m); i_ell < re_l_grid_size; i_ell++)
        {
            if ((i_proc == 0) && (i_ell % 100 == 0))
                logAdd("%d ell of %d complete\n", i_ell, re_l_grid_size);
            fill_wigner_prefactors(i_ell, re_initial_m, wigner_pre);
            for (long i_k = 0; i_k < num_k_proc; i_k++)
            {
                const double k = k_values[i_k];
                fill_bessel_array(k, R_tsurff, j_lkr);
                ...
                for (long i_ell1 = labs(re_initial_m); i_ell1 < re_l_grid_size; i_ell1++)
                {
                    // $c_{\ell, m}=\sqrt{\frac{(\ell+1)^2-m^2}{(2\ell+1)(2\ell+3)}}$
                    double c_ell1_m = c_ell_m(i_ell1, re_initial_m);
                    double c_ell1m1_m = c_ell_m(i_ell1 - 1, re_initial_m);
                    // \frac{d}{dr} f_{\ell}(kr)&=-kf_{\ell+1}(kr) + \frac{\ell}{r}f_{\ell}(kr))
                    ...
                    for (long i_ell2 = 0; i_ell2 < re_l_grid_size; i_ell2++)
                    {
                        ...
                        if (w_ell1_ell2 != 0)
                        {
                            b_kl_sum += w_ell1_ell2 * pow_neg_1(re_initial_m) * pow_neg_i(i_ell1 + 1) * pow_i(i_ell2) *
                                        (cplxd(0, 2.0) * j_lkr[i_ell1] *
                                             (((i_ell1 > labs(re_initial_m)) ? c_ell1m1_m * psi_A_surff[ind_surff_prev] : 0.0)    // if \ell_1 < m this part has to be zero
                                              + (((i_ell1 + 1) < re_l_grid_size) ? c_ell1_m * psi_A_surff[ind_surff_next] : 0.0)) // if \ell_1 = N_{\ell}-1 there is no \ell+1 orbital
                                         + j_lkr[i_ell1] * (psi_deriv_surff[ind_surff] - psi_surff[ind_surff] / R_tsurff) - j_lkr_deriv * psi_surff[ind_surff]);
                        };
                    };
                };
                partial_amplitude[index_ell_k(i_ell, i_k)] = b_kl_sum * R_tsurff * dt / sqrt(2. * M_PI);
#ifndef EXTRA_TIME_TSURFF_ONLY
                {
...
#ifndef EXTRA_TIME_TSURFF_ONLY
        {
            ...
            {
                fscanf_status = fscanf(file, "%lf %lf", &energy, &k);
                for (long ell = 0; ell < re_l_grid_size; ell++)
                {
                    fscanf_status = fscanf(file, "%lf %lf %lf %lf", &psi_re, &psi_im, &d_psi_dr_re, &d_psi_dr_im);
                };
            };
...
#endif
        if (i_proc == 0)
            logAdd("All partial amplitudes complete.\n");
    };
    ...
    */
    void eval_spectrum_34_ell1_ell2()
    {
        ...
        for (long i_k = 0; i_k < num_k_proc; i_k++)
        {
            ...
            for (long i_theta = 0; i_theta < num_theta_surff; i_theta++)
            {
                ...
                for (long i_ell = labs(re_initial_m); i_ell < re_l_grid_size; i_ell++)
                {
                    const cplxd Ylm = Ylm_prefactor(re_initial_m) * gsl_sf_legendre_sphPlm(i_ell, labs(re_initial_m), cos_theta);
                ...
                tsurff_polar_dat << k * k * 0.5 << " " << k << " " << theta << " "
                                 << "0"
                                 << " " << norm(b_k) * k << " " << real(b_k) << " " << imag(b_k) << endl;
#ifndef EXTRA_TIME_TSURFF_ONLY
                {
                    isurfv_polar_dat << k * k * 0.5 << " " << k << " " << theta << " "
                                     << "0"
                                     << " " << norm(b_k_isurfv) * k << " " << real(b_k_isurfv) << " " << imag(b_k_isurfv) << endl;
                };
#endif
            };
            // Line break for every new theta
    ...
    {
        const long re_l_grid_size_2 = re_l_grid_size * re_l_grid_size;
        return ((i_ell + 1) * (i_ell + 1) - (i_ell + 1) + i_m) + i_k * re_l_grid_size_2;
    };

    inline long index_ell1_m1_ell2_m2(long i_ell1, long i_m1, long i_ell2, long i_m2) const
    {
        const long re_l_grid_size_2 = re_l_grid_size * re_l_grid_size;
        return ((i_ell1 + 1) * (i_ell1 + 1) - (i_ell1 + 1) + i_m1) + ((i_ell2 + 1) * (i_ell2 + 1) - (i_ell2 + 1) + i_m2) * re_l_grid_size_2;
    };
    ...
    {
        const long re_l_grid_size_2 = re_l_grid_size * re_l_grid_size;
        return (i_ell1 * (i_ell1 + 1) + i_m1) + i_ell2 * re_l_grid_size_2;
    };
...
#define TESTS
    ...
    */
    void eval_partial_spectra_44_ell1_ell2()
    {
        ...
        double_ptr wigner_pre(new double[re_l_grid_size * re_l_grid_size * re_l_grid_size]);
        for (long i_ell = 0; i_ell < re_l_grid_size; i_ell++)
        {
            if ((i_proc == 0) && (i_ell % 100 == 0))
                logAdd("%d ell of %d complete\n", i_ell, re_l_grid_size);
            ...
            for (long i_m = -(i_ell); i_m < (i_ell + 1); i_m++)
            {
                ...
                for (long i_k = 0; i_k < num_k_proc; i_k++)
                {
                    ...
                    partial_amplitude[index_ell_m_k(i_ell, i_m, i_k)] = b_klm_sum * 2.0 * R_tsurff * dt;
#ifndef EXTRA_TIME_TSURFF_ONLY
                    {
                ...
                for (long ell = 0; ell < re_l_grid_size; ell++)
                {
                    ...
                    for (long i_m = -(ell); i_m < (ell + 1); i_m++)
                    {
                        ...
                        const cplxd partial_amplitude_analytic = R_tsurff / sqrt(2. * M_PI) * pow_neg_i(ell + 1) *
                                                                 (j_lkr[ell] * (psi_deriv_R_inf - psi_R_inf / R_tsurff) - j_lkr_deriv * psi_R_inf) * cplxd(0.0, 1.0) * exp(cplxd(0.0, 1.0) * energy * duration);
    ...
    */
    ...
    {
        if (i_proc == 0)
            logAdd("Obtaining angle-resolved spetrum.");
        ofstream tsurff_polar_dat(dir_name + ts_fname + "-polar" + fixed_length_number(num_proc, i_proc) + ".dat");
        tsurff_polar_dat.precision(17);
        ofstream isurfv_polar_dat(dir_name + is_fname + "-polar" + fixed_length_number(num_proc, i_proc) + ".dat");
        isurfv_polar_dat.precision(17);
        ...
        {
            if ((i_proc == 0) && (i_k % 100 == 0))
                logAdd("%d energies of %d complete\n", i_k, num_k_proc);
            const double k = k_values[i_k];
            for (long i_theta = 0; i_theta < num_theta_surff; i_theta++)
            {
                const double theta = thetas_surff[i_theta];
...
public:
    ...
    void print_Plms()
    {
        if (i_proc == 0)
        {
            ofstream debug_Plm_dat(dir_name + ts_fname + "debug_Plm.dat");
            long num_Plm_one_theta = gsl_sf_legendre_array_size(re_l_grid_size, re_initial_m);
            ...
            for (long i_theta = 0; i_theta < num_theta_surff; i_theta++)
            {
                const double cos_theta = cos(thetas_surff[i_theta]);
                const double theta = (thetas_surff[i_theta]);
                // gsl_sf_legendre_sphPlm_array(re_l_grid_size, re_initial_m, cos_theta, &(Plm_theta[i_theta*num_Plm_one_theta]));
                for (long i_ell = labs(re_initial_m); i_ell < re_l_grid_size - 1; i_ell++)
                {
                    // const long ind_Plm=i_ell-labs(re_initial_m)+num_Plm_one_theta*i_theta;
                    const double gsl_Plm = gsl_sf_legendre_sphPlm(i_ell, labs(re_initial_m), cos_theta);
                    // const cplxd dieter_Plm = ylm2(i_ell, re_initial_m, theta, 0.0);
    ...
    void print_bessel()
    {
        if (i_proc == 0)
        {
            ofstream debug_bessel_dat(dir_name + ts_fname + "debug_bessel.dat");
            double j_l2kalpha[re_l_grid_size];
            double j_l2kalpha_neg[re_l_grid_size];
            for (long i_k = 0; i_k < num_k_surff; i_k++)
            {
                const double k = k_values[i_k];
                fill_bessel_array(k, 1.0, j_l2kalpha);
                fill_bessel_array(k, -1.0, j_l2kalpha_neg);
                for (long i_ell_2 = 0; i_ell_2 < re_l_grid_size; i_ell_2++)
                {
    ...
    void print_int_dt_psi(long i_theta = 0)
    {
        ...
        {
            ofstream tsurff_input_dat(dir_name + ts_fname + "tsurff-int-dt-psi-" + to_string(i_proc) + ".dat");
            const double cos_theta = cos(thetas_surff[i_theta]);
            for (long i_k = 0; i_k < num_k_proc; i_k++)
            {
                const double k = k_values[i_k];
    ...
    void print_wigner()
    {
        if (i_proc == 0)
        {
            ofstream debug_wigner_dat(dir_name + ts_fname + "debug_wigner.dat");
            if (qprop_dim == 34)
            {
                double_ptr wigner_pre(new double[re_l_grid_size * re_l_grid_size]);
                for (long i_ell = 0; i_ell < re_l_grid_size; i_ell++)
